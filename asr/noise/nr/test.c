...
/*
test.bin in.pcm out.pcm
test.bin in.wav out.wav
sox in.wav -t raw -c 1 -e signed-integer -b 16 -r 16000 - | test.bin -t raw - out.pcm
sox in.wav -t raw -c 1 -e signed-integer -b 16 -r 16000 - | test.bin -t raw - -t raw - | sox -t raw -c 1 -e signed-integer -b 16 -r 16000 out.wav
*/
...
int main(int argc, char **argv) {
    ...
    if ((dot != NULL && !strncmp(dot + 1, "wav", 3)) && inwav) {
        fread(in, sizeof(short), 22, file_input);
        inwav = 1;
    } else if (inwav == 1) {
        fread(wav_head, sizeof(short), 22, file_input);
    }
