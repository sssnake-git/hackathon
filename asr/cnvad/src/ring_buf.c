...
{
    ringbuf_t rb = malloc(sizeof(struct s_ringbuf_t));
    if (rb) {
...
void *ringbuf_remove_oldest(ringbuf_t src, size_t count) {
    ...
    const uint8_t *src_bufend = ringbuf_end(src);
    size_t nremoved = 0;
    while (nremoved != count) {
        size_t nsrc = MIN(src_bufend - src->tail, count - nremoved);
        src->tail += nsrc;
        nremoved += nsrc;
        /* wrap ? */
        if (src->tail == src_bufend) {
...
void *ringbuf_memcpy_from_without_remove(void *dst, ringbuf_t src, size_t count) {
    ...
    const uint8_t *bufend = ringbuf_end(src);
    size_t nwritten = 0;
    uint8_t *temp_tail = src->tail;
    while (nwritten != count) {
        assert(bufend > temp_tail);
        size_t n = MIN(bufend - temp_tail, count - nwritten);
        memcpy(u8dst + nwritten, temp_tail, n);
        temp_tail += n;
        nwritten += n;
