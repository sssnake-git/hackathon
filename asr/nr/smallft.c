...
void spx_drft_init(struct drft_lookup *l, int n) {
    ...
    memset(trigcache_g, 0, (3 * 512 * sizeof(float)));
    memset(splitcache_g, 0, (32 * sizeof(int)));
    l->trigcache =
        trigcache_g;  //(float*)speex_alloc(3 * n * sizeof(*l->trigcache));
    l->splitcache =
        splitcache_g;  //(int*)speex_alloc(32 * sizeof(*l->splitcache));
    fdrffti(n, l->trigcache, l->splitcache);
}

void spx_drft_clear(struct drft_lookup *l) {
    if (l) {
        if (l->trigcache) speex_free(l->trigcache);
        if (l->splitcache) speex_free(l->splitcache);
    }
}
